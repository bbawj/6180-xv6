...
char *mbufpush(struct mbuf *m, unsigned int len) {
  m->head -= len;
  if (m->head < m->buf) panic("mbufpush");
  m->len += len;
  return m->head;
}
...
static void net_rx_udp(struct mbuf *m, uint16 len, struct ip *iphdr) {
  ...
  if (ntohs(udphdr->ulen) != len) goto fail;
  len -= sizeof(*udphdr);
  if (len > m->len) goto fail;
...
static void net_rx_ip(struct mbuf *m) {
  ...
  if (iphdr->ip_vhl != ((4 << 4) | (20 >> 2))) goto fail;
  // validate IP checksum
  if (in_cksum((unsigned char *)iphdr, sizeof(*iphdr))) goto fail;
  // can't support fragmented IP packets
  if (htons(iphdr->ip_off) != 0) goto fail;
  // is the packet addressed to us?
  if (htonl(iphdr->ip_dst) != local_ip) goto fail;
  // can only support UDP
  if (iphdr->ip_p != IPPROTO_UDP) goto fail;
