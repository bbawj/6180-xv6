...
static int dump_pagetable(pagetable_t p, int depth) {
  if (depth > 2) {
    return 0;
  }
  ...
  for (int i = 0; i < 512; ++i) {
    ...
    if (pte & PTE_V) {
      ...
      pagetable_t new_p = (pagetable_t)PTE2PA(pte);
      printf("%d: pte %p pa %p\n", i, pte, new_p);
      dump_pagetable(new_p, depth + 1);
    }
  }
  return 0;
}
...
int vmprint(pagetable_t pagetable) {
  ...
  dump_pagetable(pagetable, 0);

  return 0;
}

int pgaccess(pagetable_t pagetable, uint64 starting_page, int npages,
             uint64 bitmask_addr) {
  uint64 ret_bitmask = 0;
  for (int i = 0; i < npages; ++i) {
    pte_t *pte = walk(pagetable, starting_page + PGSIZE * i, 0);
    if (pte == 0) continue;
  ...
  return copyout(pagetable, bitmask_addr, (char *)&ret_bitmask,
                 sizeof(ret_bitmask));
}
